...
function played(game){
  const k=`ppw-games-v9-done-${game}-${day}`;if(get(k,'')==='1')return;put(k,'1');
  const d=new Date();d.setDate(d.getDate()-1);const y=today.call({});
  ...
  stats.lastDay=day;stats.played++;if(!stats.weekDays.includes(day))stats.weekDays.push(day);stats.weekDays=stats.weekDays.slice(-7);put(statsKey,JSON.stringify(stats));renderStats();
}
...
if(wordBoard&&wordMsg&&keyboard){
 ...
 function paint(){wordBoard.querySelectorAll('.word-cell').forEach(e=>{e.textContent='';e.className='word-cell'});guesses.forEach((g,r)=>{[...g].forEach((l,c)=>{const e=wordBoard.querySelector(`[data-r="${r}"][data-c="${c}"]`);e.textContent=l;e.classList.add('filled',l===answer[c]?'correct':answer.includes(l)?'present':'absent')})});if(current&&!finished)[...current].forEach((l,c)=>{const e=wordBoard.querySelector(`[data-r="${guesses.length}"][data-c="${c}"]`);e.textContent=l;e.classList.add('filled','pop')})}
 ...
 function keyColors(){const rank={};guesses.forEach(g=>[...g].forEach((l,i)=>{const n=l===answer[i]?3:answer.includes(l)?2:1;rank[l]=Math.max(rank[l]||0,n)}));Object.entries(rank).forEach(([l,n])=>{keys[l].className=`key ${n===3?'correct':n===2?'present':'absent'}`})}
...
if(grid){
 const puzzles=[
  {down:'TIGER',across:[['PLATE','A flat dish used for food.'],['AGAIN','Once more.'],['STAGE','Where performers may perform.'],['ANGEL','A heavenly messenger.'],['SHARE','To give part of something to others.']]},
  {down:'CROWN',across:[['PLACE','A location or spot.'],['HEART','The organ that pumps blood.'],['FLOOR','The surface you walk on indoors.'],['THROW','To send something through the air.'],['TRAIN','A long vehicle that runs on tracks.']]},
  {down:'SMART',across:[['ARISE','To get up or appear.'],['STORM','A period of strong weather.'],['CREAM','A smooth dairy food.'],['SCORE','Points earned in a game.'],['PLATE','A flat dish used for food.']]}
 ];
 const week=Math.floor((Date.now()-Date.UTC(2026,0,1))/(7*86400000));const puzzle=puzzles[((week%puzzles.length)+puzzles.length)%puzzles.length];const key=`ppw-crossword-v9-${week}`;let saved=(()=>{try{return JSON.parse(get(key,'[]'))}catch{return[]}})();const inputs=[];grid.innerHTML='';
...
if(memory){
 const symbols=['🐯','★','✦','●','◆','▲','✿','☀'];let cards=[],open=[],matches=0,moves=0,locked=false,start=0;
 function shuffle(a){for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}return a}
 function setup(){memory.innerHTML='';open=[];matches=0;moves=0;locked=false;start=performance.now();movesEl&&(movesEl.textContent='0');memoryMsg&&(memoryMsg.textContent='Find all eight pairs.',memoryMsg.className='game-message');cards=shuffle([...symbols,...symbols]);cards.forEach((symbol,i)=>{const b=document.createElement('button');b.type='button';b.className='memory-card';b.innerHTML=`<span class="memory-back">?</span><span class="memory-face" aria-hidden="true">${symbol}</span>`;b.setAttribute('aria-label','Hidden card');b.addEventListener('click',()=>flip(b,symbol));memory.appendChild(b)})}
 function flip(card,symbol){if(locked||card.classList.contains('flipped')||card.classList.contains('matched'))return;card.classList.add('flipped');open.push({card,symbol});if(open.length<2)return;moves++;movesEl&&(movesEl.textContent=moves);if(open[0].symbol===open[1].symbol){open.forEach(x=>x.card.classList.add('matched'));matches+=2;open=[];if(matches===cards.length){const time=performance.now()-start;if(!stats.best||time<stats.best){stats.best=time;put(statsKey,JSON.stringify(stats));renderStats()}played('memory');memoryMsg.textContent=`Perfect! ${moves} moves in ${(time/1000).toFixed(1)} seconds.`;memoryMsg.className='game-message win';celebrate(memory)}}else{locked=true;memory.classList.add('memory-miss');setTimeout(()=>{open.forEach(x=>x.card.classList.remove('flipped'));open=[];locked=false;memory.classList.remove('memory-miss')},650)}}
