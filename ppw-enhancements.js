...
function newsroom(){const credit=document.querySelector('.staff-section .credit-card');if(credit)credit.remove();const grid=document.getElementById('staffGrid');if(grid){grid.setAttribute('aria-label','PPW Staff Directory');const observer=new MutationObserver(()=>{grid.querySelectorAll('.staff-card').forEach(card=>card.classList.add('newsroom-member'));reveal()});observer.observe(grid,{childList:true,subtree:true})}}
function start(){favicon();styles();addGamesNav();sharedTheme();newsroom();reveal()}
